...
@section('main')
		...
		<div class="panel panel-default">
					                <div class="panel-heading">
					                	<h6 class="panel-title"><i class="fas fa-chalkboard-student"></i>QUẢN LÝ GIÁO VIÊN</h6></div>
					                <div class="datatable">
					                	@if(session('thongbao'))
		 ...
		 <table class="table tbl" >
				                    ...
				                    <tbody>
										...
										<tr>
											...
											<td >{{$gv->diachi}}</td>
											<td >{{$gv->sdt}}</td>
											<td id="name" name="email">{{$gv->email}}</td>		
											<td><a  href="admin/giaovien/suagiaovien/{{$gv->id_gv}}">
													<button data-toggle="modal"  class="btn btnsuach" >Sửa</button>
												</a>
											</td>
											<td><a onclick="return confirm('Bạn có chắc chắn muốn xóa không?')"
												href="admin/giaovien/xoagiaovien/{{$gv->id_gv}}">
