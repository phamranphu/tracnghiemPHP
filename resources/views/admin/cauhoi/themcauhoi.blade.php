...
@section('main')
            <div class="panel panel-default khung">
				...
				<select name="namemd" id="txtMD" class="form-control">
					...
					<select name="dapandung" class="form-control myanswer">
...
<script >
	...
	$('#tenloaich').change(function(){
		// alert($(this).val())
		if($(this).val() == '4'){
			var optionSelect = "<option value='A'>A</option><option value='B'>B</option>";
			$('.myanswer').html(optionSelect);

		}
		else{
			var optionSelect = "<option value='A'>A</option><option value='B'>B</option><option value='C'>C</option><option value='D'>D</option>";
			$('.myanswer').html(optionSelect);
		}
	})
</script>	          
